/**
 * @file deportista.c
 * @brief Funciones para la generación, manejo y almacenamiento en CSV de deportistas.
 */
...
/**
 * @brief Genera un nombre aleatorio concatenando sílabas al azar.
 * @param destino Puntero a la cadena donde se guardará el nombre generado.
 * @param longitud_maxima Longitud máxima permitida para el nombre.
 */
...
/**
 * @brief Reinicia el contador de ID autoincremental a 1.
 */
...
/**
 * @brief Crea un deportista con valores aleatorios y le asigna el siguiente ID autoincremental.
 * @return Un objeto de tipo Deportista completamente inicializado.
 */
...
/**
 * @brief Crea un deportista con valores aleatorios, pero asignándole un ID fijo provisto.
 * @param id El identificador único a asignarle al deportista.
 * @return Un objeto de tipo Deportista con dicho ID.
 */
...
/**
 * @brief Genera un arreglo de deportistas ordenados de menor a mayor por su ID (Mejor caso).
 * @param arreglo Arreglo en el cual se almacenarán los deportistas.
 * @param cantidad Cantidad de deportistas a generar.
 */
...
/**
 * @brief Genera un arreglo de deportistas ordenados inversamente por su ID (Peor caso).
 * @param arreglo Arreglo en el cual se almacenarán los deportistas.
 * @param cantidad Cantidad de deportistas a generar.
 */
...
/**
 * @brief Aplica el Algoritmo de Fisher-Yates para mezclar aleatoriamente el arreglo. (Caso promedio).
 * @param arreglo Arreglo de deportistas a mezclar.
...
/**
 * @brief Exporta el arreglo de deportistas a un archivo CSV.
 * @param arreglo Arreglo de deportistas a exportar.
 * @param cantidad Cantidad de elementos a escribir.
 * @param nombre_archivo Ruta o nombre del archivo CSV destino.
 */
...
/**
 * @brief Lee los registros de un archivo CSV y los carga en el arreglo en memoria.
 * @param arreglo Arreglo destino en memoria.
 * @param cantidad_maxima Límite máximo de registros a leer.
 * @param nombre_archivo Ruta o nombre del archivo CSV origen.
 * @return int Cantidad de registros leídos y cargados exitosamente.
 */
int leer_deportistas_csv(Deportista *arreglo, int cantidad_maxima, const char *nombre_archivo) {
    ...
    while (fgets(linea, sizeof(linea), archivo) != NULL && contador < cantidad_maxima) {
        if (sscanf(linea, "%d,%[^,],%[^,],%f,%d", &arreglo[contador].id, arreglo[contador].nombre, arreglo[contador].equipo, &arreglo[contador].puntaje, &arreglo[contador].competencias) == 5) {
            contador++;
        }
    }
...
/**
 * @brief Imprime en la consola la información de un deportista estructurada en formato tabla.
 * @param d Deportista que se desea imprimir en pantalla.
 */
